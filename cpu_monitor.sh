...
show_help_en() {
    ...
    echo ""
    echo "Parameters:"
    echo "  -t <threshold>      CPU usage threshold in percentage (default: $THRESHOLD)"
    echo "  -d <duration>       Duration in seconds above the threshold before triggering an alert (default: $DURATION_THRESHOLD seconds)"
    echo "  -i <interval>       Check interval in seconds (default: $INTERVAL seconds)"
    echo "  -l <language>       Alert language: EN (English) or ES (Spanish) (default: $LANGUAGE)"
...
show_help_es() {
    ...
    echo ""
    echo "Parámetros:"
    echo "  -t <umbral>         Umbral de uso de CPU en porcentaje (por defecto: $THRESHOLD)"
    echo "  -d <duración>       Duración en segundos por encima del umbral antes de activar una alerta (por defecto: $DURATION_THRESHOLD segundos)"
    echo "  -i <intervalo>      Intervalo de comprobación en segundos (por defecto: $INTERVAL segundos)"
    echo "  -l <idioma>         Idioma de las alert"
